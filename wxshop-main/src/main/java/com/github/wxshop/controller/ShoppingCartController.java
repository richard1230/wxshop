...
public class ShoppingCartController {
    ...
    public PageResponse<ShoppingCartData> getShoppingCart(
        ...
        return shoppingCartService.getShoppingCartOfUser(
                UserContext.getCurrentUser().getId(), pageNum, pageSize);
    }
     ...
     * @apiSuccessExample Success-Response: HTTP/1.1 200 OK { "data": { "shop": { "id": 12345, "name":
    ...
    public Response<ShoppingCartData> addToShoppingCart(
            @RequestBody AddToShoppingCartRequest request) {
        return Response.of(
                shoppingCartService.addToShoppingCart(request, UserContext.getCurrentUser().getId()));
    }
    ...
    public Response<ShoppingCartData> deleteGoodsInShoppingCart(@PathVariable("id") Long goodsId) {
        return Response.of(
                shoppingCartService.deleteGoodsInShoppingCart(
                        goodsId, UserContext.getCurrentUser().getId()));
    }
}
